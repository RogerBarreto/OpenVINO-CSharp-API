...
namespace OpenVinoSharp.Extensions.model
{
    public class Yolov5Seg : Predictor
    {
        ...
        public List<SegResult> process_result(float[] detect, float[] proto, int batch)
        {
            ...
            {
                Mat detect_data = new Mat( m_output_length, 37 + m_categ_nums, DepthType.Cv32F, 1,
                    Marshal.UnsafeAddrOfPinnedArrayElement(detect, (37 + m_categ_nums) * m_output_length * b * 4), 4 * (37 + m_categ_nums));
                ...
                {
                    Mat mat = new Mat(detect_data, new Rectangle(0, i, 5, 1));
                    ...
                    Mat classes_scores = new Mat(detect_data, new Rectangle(5, i, m_categ_nums, 1));// GetArray(i, 5, classes_scores);
                    ...
                    if (max_score > 0.25)
                    {
                        ...
                        Mat mask = new Mat(detect_data, new Rectangle(5 + m_categ_nums, i, 32, 1));//detect_data.Row(i).ColRange(4 + categ_nums, categ_nums + 36);
                        float cx = data[0, 0];
                        float cy = data[0, 1];
                        float ow = data[0, 2];
                        float oh = data[0, 3];
